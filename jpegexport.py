...
# except:
#     inkex.errormsg(
#         _('The export_gpl.py module requires PyXML.  Please download the latest version from http://pyxml.sourceforge.net/.'))
#     sys.exit()
...
# stream.close()
# # walk(dom)
# myDocName = dom.getElementsByTagName(
#     'svg')[0].getAttribute('sodipodi:docname').split('.')[0]
...
class JPEGExport(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option(
            "--path",    action="store", type="string",  dest="path",    default="",        help="")
        self.OptionParser.add_option(
            "--bgcol",   action="store", type="string",  dest="bgcol",   default="#ffffff", help="")
        self.OptionParser.add_option(
            "--quality", action="store", type="int",     dest="quality", default="90",      help="")
        self.OptionParser.add_option(
            "--density", action="store", type="int",     dest="density", default="90",      help="")
        self.OptionParser.add_option(
            "--page",    action="store", type="inkbool", dest="page",    default=False,     help="")
        self.OptionParser.add_option(
            "--fast",    action="store", type="inkbool", dest="fast",    default=True,      help="")
    ...
    def effect(self):
        ...
        if not self.options.path
            ...
            jpgname = ('%s.jpg') % docname

            home = expanduser("~")
            picturesFolder = ('%s/Pictures') % home

            fullPath = ('%s/%s') % (picturesFolder, jpgname)

            self.options.path = fullPath
            # inkex.errormsg(
            #     _('docname: %s | %s') % (os.path.basename(self.options.path), os.path.dirname(self.options.path)))
            # exit()

        # The user must supply a directory to export:
        if not self.options.path:
            inkex.errormsg(
                _('Please indicate a file name and path to export the jpg. "%s"') % inkex.Effect().document)
            exit()
        ...
        if not os.path.dirname(self.options.path):
            inkex.errormsg(
                _('Please indicate a directory other than your system\'s base directory.'))
            exit()

        # Test if the directory exists:
        if not os.path.exists(os.path.dirname(self.options.path)):
            inkex.errormsg(_('The directory "%s" does not exist.') %
                           os.path.dirname(self.options.path))
            exit()
        ...
        if not _rgbhexstring.match(self.options.bgcol):
            inkex.errormsg(
                _('Please indicate the background color like this: \"#abc123\" or leave the field empty for white.'))
            exit()
        ...
        if self.options.page == False:
            ...
            self.exportArea(int(coords[0]), int(coords[1]), int(
                coords[2]), int(coords[3]), curfile, outfile, bgcol)
            ...
            if self.options.fast == True:  # uses simpletransform
                ...
                for prop in props:
                    command = ("inkscape", "--without-gui",
                               "--query-id", id, "--query-"+prop, self.args[-1])
    ...
    def exportArea(self, x0, y0, x1, y1, curfile, outfile, bgcol):
        tmp = self.getTmpPath()
        command = "inkscape -a %s:%s:%s:%s -d %s -e \"%sjpinkexp.png\" -b \"%s\" %s" % (
            x0, y0, x1, y1, self.options.density, tmp, bgcol, curfile)
    ...
    def exportPage(self, curfile, outfile, bgcol):
        tmp = self.getTmpPath()
        command = "inkscape -C -d %s -e \"%sjpinkexp.png\" -b\"%s\" %s" % (
            self.options.density, tmp, bgcol, curfile)
    ...
    def tojpeg(self, outfile):
        ...
        if find_executable('magick'):
            command = "magick -quality %s -density %s \"%sjpinkexp.png\" \"%s\" " % (
                self.options.quality, self.options.density, tmp, outfile)
        elif find_executable('convert'):
            command = "convert \"%sjpinkexp.png\" -quality %s -density %s \"%s\" " % (
                tmp, self.options.quality, self.options.density, outfile)
        else:
